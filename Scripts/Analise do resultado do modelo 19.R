...
mDirMask <- "GVAR_Toolbox2.0/Output/Meso19_%dperc"
mFileMask <- "output.xlsx"
...
mOutputGraphDir <- "Export/Graph/Results Up to 2016 (mdl19)"
...
for(i in seq_along(percent)){
  ...
  tbl <- readxl::read_excel(path = file.path(sprintf(mDirMask, percent_current), mFileMask),
...
for(i in seq_along(percent)){
  ...
  # tbl %>% filter(Region == "Sp3515")
  summary(tbl)
  
  if(i == 1){
    tblfull <- tbl 
  } else {
    tblfull <- left_join(tblfull, tbl, by = c("Region"="Region",
                                               "Series"="Series",
                                               "Date" = "Date") )
  }
}
...
# for(i in seq_along(names(my_label_n_level))){
  
  region <- mListRegions[i]
  # region <- names(my_label_n_level)[i]
...
graph <- tblfull %>% 
  ...
  tidyr::pivot_longer(cols = c("Perc_0", "Perc_2", "Perc_4", "Perc_6", "Perc_8", "Actual"),
  # tidyr::pivot_longer(cols = c("Perc_0", "Perc_2", "Perc_4", "Actual"),
